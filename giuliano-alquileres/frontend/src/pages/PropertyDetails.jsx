        ...
        {photos.length > 0 ? (
          <div className="grid grid-cols-4 grid-rows-2 gap-2 h-[500px] rounded-2xl overflow-hidden">
            ...
            <div className="col-span-2 row-span-2 relative group cursor-pointer" onClick={() => setShowAllPhotos(true)}>
              <img
                src={photos[0].cloudinary_url || `${import.meta.env.VITE_API_URL || 'https://giuliano.onrender.com'}/uploads/properties/${photos[0].filename}`}
                alt={property.title}
            ...
            {photos.slice(1, 5).map((photo, index) => (
              ...
              >
                <img
                  src={photo.cloudinary_url || `${import.meta.env.VITE_API_URL || 'https://giuliano.onrender.com'}/uploads/properties/${photo.filename}`}
                  alt={`Foto ${index + 2}`}
                  className="w-full h-full object-cover group-hover:brightness-90 transition-all"
                />
